...
class NewHtmlFormatter < RSpec::Core::Formatters::BaseFormatter
  ...
  def remove_ascii_colors string
    string.gsub(/\e\[([;\d]+)?m/, '')
  end
  ...
  def example_failed(failure)
    ...
    #urls = $request_uri.join(", ")
    
    #blah = failure.fully_formatted(nil, RSpec::Core::Notifications::NullColorizer)
    exception_details = if exception
                          {
                            :message => remove_ascii_colors(failure.message_lines.join("\n")),
                            :backtrace => remove_ascii_colors(failure.formatted_backtrace.join("\n")),
                            :request_uri => "HTTP Request URI: #{$request_uri}\n"
                          }
                        end
...
private
  ...
  def extra_failure_content(failure)
    require_relative "html_snippet_extractor"
